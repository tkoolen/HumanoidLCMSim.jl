...
function LCMController(robot_info::HumanoidRobotInfo)
    ...
    for joint in tree_joints(mechanism)
        if num_positions(joint) == 1
            push!(robot_state_msg.joint_name, string(joint))
        end
    end
    resize!(robot_state_msg)

    subscribe(lcm, "ROBOT_COMMAND", (channel, data) -> handle_robot_command_msg(controller, data))
    @async while true
        handle(lcm)
    end
...
function set!(msg::RobotStateT, result::DynamicsResult, robot_info::HumanoidRobotInfo, τprev::AbstractVector, t::Number, state::MechanismState)
    ...
    world_aligned_floating_body_frame = CartesianFrame3D() # TODO: relinquish
    to_world_aligned_floating_body = Transform3D(pose.to, world_aligned_floating_body_frame, -translation(pose))
    twist = transform(twist, to_world_aligned_floating_body)
    set!(msg.twist, twist)

    # state info for actuated joints
    for i = 1 : msg.num_joints
        joint = findjoint(robot_info, msg.joint_name[i])
